...
def classifer(encoder_embed_input,max_target_sequence_length,keep_prob=0.5,reuse=False,training = False):
...
def get_cost_c(pred): #compute classifier cost
    cost=tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(logits=pred,labels=l_y))
    return cost
...
def get_cost_u(u_encoder_embed_input,u_decoder_embed_input):
    ...
    #均值 方差
    prob_y  = classifer(u_encoder_embed_input,un_max_target_sequence_length,keep_prob=keep_prob,reuse=True,training = training)
...
pred=classifer(l_encoder_embed_input,max_target_sequence_length,keep_prob=keep_prob,training = training)

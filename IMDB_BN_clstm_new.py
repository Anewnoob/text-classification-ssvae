...
def classifer(encoder_embed_input,l_a,l_z,keep_prob=0.5,reuse=False):
    with tf.variable_scope("classifier",reuse=reuse):
        ...
        #l_c = (fw_outputs + bw_outputs)[-1]
        l_c = tf.concat(outputs, 2)[-1]
        l_ac = tf.concat([l_c,l_a],1) #a [batch a_size]
        l_ac = tf.nn.relu(tf.layers.batch_normalization(l_ac, training=training, name="BN_1"))
        pred = tf.matmul(l_ac, weights_de["out"]) + biases_de["out"]
...
def get_cost_u(u_encoder_embed_input,u_decoder_embed_input):
    ...
    #均值 方差
    prob_y  = classifer(u_encoder_embed_input,l_a,l_z,keep_prob=keep_prob,reuse=True)
...
pred=classifer(l_encoder_embed_input,l_a,l_z,keep_prob=keep_prob)

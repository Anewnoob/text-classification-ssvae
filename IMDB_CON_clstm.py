...
def encoder(l_encoder_embed_input,keep_prob=0.5, reuse=False):
    with tf.variable_scope("encoder", reuse=reuse):
        ...
        l_x_to_a = tf.contrib.layers.fully_connected(inputs=l_x, num_outputs=a_size, activation_fn=tf.nn.tanh,
                                                     scope="x-to-a")
        a_mean = tf.contrib.layers.fully_connected(inputs=l_x_to_a, num_outputs=a_size, activation_fn=None,
                                                   scope="a_mean")
        a_stddev = tf.contrib.layers.fully_connected(inputs=l_x_to_a, num_outputs=a_size, activation_fn=None,
                                                     scope="a_std")
        ...
        z_stddev = tf.contrib.layers.fully_connected(inputs=l_axy, num_outputs=z_size, activation_fn=None,
                                                     scope="z_std")
...
def decoder(decoder_embed_input, decoder_y, target_length, max_target_length, l_z, l_y, states, keep_prob, reuse=False):
    with tf.variable_scope("decoder", reuse=reuse):
        ...
        decode_lstm = clstm.LSTMCell(n_hidden,label_size,n_input,forget_bias=1.0, state_is_tuple=True)
...
def get_cost_l(l_encoder_embed_input, decoder_embed_input, l_y, decoder_y, target_sequence_length,
               max_target_sequence_length, reuse=False):
    # encoder
    # q(a|x) q(z|axy)
    l_a, l_z, states, a_mean, a_stddev, z_mean, z_stddev = encoder(l_encoder_embed_input,keep_prob, reuse,
                                                                   )

    # decoder
    training_logits, masks, u_mean, u_stddev = decoder(decoder_embed_input, decoder_y, target_sequence_length,
                                                       max_target_sequence_length, l_z, l_y, states, keep_prob, reuse)
...
def get_cost_u(u_encoder_embed_input, u_decoder_embed_input):
    ...
    for label in range(label_size):
        ...
        cost_l, l_a = get_cost_l(u_encoder_embed_input, u_decoder_embed_input, [y_i] * batch_size, vae_y_u[label],
                                 un_target_sequence_length, un_max_target_sequence_length, reuse=True)
...
def calc_kl(a_mean, a_stddev, z_mean, z_stddev, u_mean, u_stddev):
    ...
    latent_loss_qau = 0.5 * tf.reduce_sum(
        tf.log(2 * np.pi) + u_stddev + tf.square(u_mean - a_mean) / (1e-8 + tf.exp(u_stddev)) + tf.exp(a_stddev) / (
                    1e-8 + tf.exp(u_stddev)), 1)
...
cost_l, l_a = get_cost_l(l_encoder_embed_input, l_decoder_embed_input, l_y, vae_y, target_sequence_length,
...
def train_model():
    ...
    with tf.Session(config=config) as sess:
        sess.run(initial)
        saver.restore(sess, './temp/imdb_4_2500_con_lvae.pkt')
        ...
        for epoch in range(iter_num):
            ...
            while step < len(new_trainS) // batch_size:
                ...
                lu_cost, p_y, ulab, pred_batch, c_pred, op, batch_cost, c_cost, l_cost, u_cost = sess.run(
                    [cost_lu, prob_y, L_ulab, pred, correct_pred, train_op, cost, cost_c, cost_l, cost_u],
                ...
                if (step % 10 == 0 and step is not 0):
                    ...
                    print ('TRAIN LOSS', train_cost, 'LABEL COST', label_cost, 'Unlabel Cost', unlabel_cost, 'lu Cost',
                           lu_abs_cost, 'Classifier Cost', classifier_cost)
                    print ("\n")
...
def test_model(sess, testS, testL, epoch):
    ...
    while step < len(testS) // batch_size:  #
        ...
        c_pred, pred_batch = sess.run([correct_pred, pred],
                                      feed_dict={l_encoder_embed_input: sources_batch, l_y: batch_y, keep_prob: 1.0,
                                                 training: False})
        for i in range(len(pred_batch)):
            value = pred_batch[i]
            # print value

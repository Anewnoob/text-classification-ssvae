...
def decoder(decoder_embed_input,l_y,decoder_y,target_length,max_target_length,l_u,states,keep_prob=0.5,reuse=False):
    with tf.variable_scope("decoder",reuse=reuse):
        ...
        h_states = tf.nn.tanh(tf.matmul(l_yuy, weights_de['w_']) + biases_de['b_'])

        decoder_initial_state = LSTMStateTuple(states[0], h_states)   #(C,h_state)
...
def get_cost_u(u_encoder_embed_input,u_decoder_embed_input):
    ...
    for label in range(label_size):
        ...
        training_logits, masks = decoder(u_decoder_embed_input, [y_i]*batch_size,vae_y_u[label],un_target_sequence_length,

...
def encoder(l_encoder_embed_input,l_y,keep_prob=0.5,reuse=False):
    with tf.variable_scope("encoder",reuse=reuse):
        ...
        l_x = states[-1]
        l_xy = tf.concat([l_y,l_x,l_y], 1)
        l_x_to_a = tf.contrib.layers.fully_connected(inputs=l_xy, num_outputs=a_size, activation_fn=None,scope="x-to-a")

        l_x_to_a = tf.concat([l_y,l_x_to_a,l_y], 1)
        ...
        l_ay=tf.concat([l_y,l_a,l_y],1)
        z_mean = tf.contrib.layers.fully_connected(inputs=l_ay, num_outputs=z_size, activation_fn=None,scope="z_mean")
        z_stddev = tf.contrib.layers.fully_connected(inputs=l_ay, num_outputs=z_size, activation_fn=None,scope="z_std")
...
def decoder(decoder_embed_input,l_y,decoder_y,target_length,max_target_length,l_z,states,keep_prob=0.5,reuse=False):
    with tf.variable_scope("decoder",reuse=reuse):
        ...
        l_yu = tf.concat([l_y,l_u,l_y], 1)
        h_states = tf.nn.softplus(tf.matmul(l_yu, weights_de['w_']) + biases_de['b_'])
...
def get_cost_u(u_encoder_embed_input,u_decoder_embed_input):
    for label in range(label_size):
        ...
        l_a, l_z, states, a_mean, a_stddev, z_mean, z_stddev = encoder(u_encoder_embed_input,[y_i]*batch_size,keep_prob=keep_prob,
                                                                       reuse=True)
    ...
    #均值 方差
    #mean,variance = tf.nn.moments(L_ulab,1)
...
with tf.name_scope("weight_inital"):
    weights_de={
        'w_':tf.Variable(tf.random_normal([2*label_size+a_size,n_hidden],mean=0.0, stddev=0.01)),
...
l_a,l_z,states, a_mean, a_stddev,z_mean,z_stddev= encoder(l_encoder_embed_input,l_y,keep_prob)
training_logits,masks,u_mean,u_stddev = decoder(l_decoder_embed_input,l_y,vae_y,target_sequence_length,max_target_sequence_length,l_z,states,keep_prob)
    ...
    with tf.Session(config=config) as sess:
        ...
        for epoch in range(iter_num):
            ...
            while step < len(new_trainS) // batch_size:
                ...
                ulab,p_y,lu_cost,pred_batch,c_pred,op,batch_cost,c_cost,l_cost,u_cost=sess.run([L_ulab,prob_y,cost_lu,pred,correct_pred,train_op,cost,cost_c,cost_l,cost_u],

...
def classifer(encoder_embed_input,max_target_sequence_length,keep_prob=0.5,reuse=False):
    with tf.variable_scope("classifier",reuse=reuse):
        ...
        #l_c = tf.nn.relu(tf.layers.batch_normalization(r, training=training, name="BN_2"))
...
def encoder(l_encoder_embed_input,keep_prob=0.5,reuse=False):
    with tf.variable_scope("encoder",reuse=reuse):
        ...
        l_x = states[-1]
        l_x_to_a = tf.contrib.layers.fully_connected(inputs=l_x, num_outputs=a_size, activation_fn=None,scope="x-to-a")
        ...
        l_xz = tf.concat([l_x,l_z], 1)
        u_mean = tf.contrib.layers.fully_connected(inputs=l_xz, num_outputs=a_size, activation_fn=None,scope="u_mean")
        u_stddev = tf.contrib.layers.fully_connected(inputs=l_xz, num_outputs=a_size, activation_fn=None,scope="u_std")

        samples = tf.random_normal(tf.shape(u_stddev))
        l_u = u_mean + tf.exp(u_stddev * 0.5) * samples

        return l_a,l_z,l_u,states, a_mean, a_stddev,z_mean,z_stddev,u_mean,u_stddev

def decoder(decoder_embed_input,l_y,decoder_y,target_length,max_target_length,l_u,states,batch_size,keep_prob=0.5,reuse=False):
    with tf.variable_scope("decoder",reuse=reuse):
        l_yuy = tf.concat([l_u,l_y], 1)
        h_states = tf.nn.softplus(tf.matmul(l_yuy, weights_de['w_']) + biases_de['b_'])
...
def get_cost_l(decoder_embed_input,a_mean, a_stddev, z_mean, z_stddev,training_logits, masks,u_mean,u_stddev):
...
def get_cost_u(u_encoder_embed_input,u_decoder_embed_input):
    l_a, l_z, l_u, states, a_mean, a_stddev, z_mean, z_stddev, u_mean, u_stddev = encoder(u_encoder_embed_input,keep_prob=keep_prob,
                                                                   reuse=True)
    for label in range(label_size):
        y_i=get_onehot(label)
        vae_y = tf.to_float(tf.convert_to_tensor([y_i] * un_batch_size))
        training_logits, masks = decoder(u_decoder_embed_input, vae_y,vae_y_u[label],un_target_sequence_length,
                                                           un_max_target_sequence_length, l_u, states, un_batch_size,keep_prob,reuse=True)
...
with tf.name_scope("weight_inital"):
    weights_de={
        'w_':tf.Variable(tf.random_normal([label_size+u_size,n_hidden],mean=0.0, stddev=0.01)),
    ...
    'out2': tf.Variable(tf.random_normal([label_size]))
    }
...
l_a,l_z,l_u,states, a_mean, a_stddev,z_mean,z_stddev,u_mean,u_stddev= encoder(l_encoder_embed_input,keep_prob)
training_logits,masks = decoder(l_decoder_embed_input,l_y,vae_y,target_sequence_length,max_target_sequence_length,l_u,states,batch_size,keep_prob)
...
#alpha_c = 1 * 1.5 * ( Train_Size + 25000 ) / Train_Size
cost=tf.reduce_mean(cost_l*2500)+tf.reduce_mean(cost_u*25000)
        ...
        for epoch in range(iter_num):
            ...
            while step < len(new_trainS) // batch_size:
                ...
                for source in input_ux:
                    un_pad_source_lengths.append(len(source)+1)  # +1
                #label
                pad_source_lengths = []
                ...
                p_y,ulab,pred_batch,c_pred,op,batch_cost,c_cost,l_cost,u_cost=sess.run([prob_y,L_ulab,pred,correct_pred,train_op,cost,cost_c,cost_l,cost_u],
    ...
    while step < len(testS) // batch_size:  #
        ...
        f_pred,c_pred,pred_batch=sess.run([correct_pred_f,correct_pred,pred],feed_dict={l_encoder_embed_input:sources_batch,l_y: batch_y,keep_prob: 1.0,
                                                                  target_sequence_length: pad_source_lengths,
                                                                                        l_decoder_embed_input: input_batch,
                                                                                        training:False})
        for i in range(len(pred_batch)):
            value = pred_batch[i]
            #print value

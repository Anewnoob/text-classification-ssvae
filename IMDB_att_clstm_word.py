...
neg_topic_index = vocab_to_int['good']
pos_topic_index = vocab_to_int['bad']
...
def creat_u_topic_scopus(seq_length): #unlabel data
    ucon_y=[]
    for i in range(label_size):
        if i == 0:
            topic_step = neg_topic_index
        elif i == 1:
            topic_step = pos_topic_index
        temp=[]
        for j in range(batch_size):
            temp.append(creat_y_scopus([topic_step]*label_size, seq_length))
        ucon_y.append(temp)
    return ucon_y
...
topic=tf.placeholder(dtype=tf.float32,shape=[batch_size,label_size])
vae_topic=tf.placeholder("float",[batch_size,None,label_size])
vae_topic_u=tf.placeholder("float",[label_size,batch_size,None,label_size])
        ...
        for epoch in range(iter_num):
            ...
            while step < len(new_trainS) // batch_size:
                ...
                for y_i in range(start_i, start_i + batch_size):
                    label_index = get_mask_index(new_trainL[y_i], label_list)
                    if label_index == 0:
                        topic_step = neg_topic_index
                    elif label_index ==1:
                        topic_step = pos_topic_index
                    ...
                    decode_topic.append(creat_y_scopus([topic_step]*label_size, target_maxlength))
                    batch_y.append(xsy_step)
                    batch_topic.append([topic_step]*label_size)
                decode_uy=creat_u_y_scopus(un_target_maxlength)
                decode_utopic = creat_u_topic_scopus(un_target_maxlength)
                ...
                lu_cost,p_y,ulab,pred_batch,c_pred,op,batch_cost,c_cost,l_cost,u_cost=sess.run([cost_lu,prob_y,L_ulab,pred,correct_pred,train_op,cost,cost_c,cost_l,cost_u],
                                                                              ...
                                                                              l_y:batch_y,training:True,topic:batch_topic,vae_topic:decode_topic,vae_topic_u:decode_utopic,

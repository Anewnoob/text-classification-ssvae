    ...
    with tf.variable_scope("classifier", reuse=reuse):
        with tf.variable_scope("classifier", reuse=reuse):
            ...
            alpha = tf.nn.softmax(tf.reshape(tf.matmul(tf.reshape(M, [-1, c_hidden]),
                                                       tf.reshape(W, [-1, 1])),
                                             (-1, max_target_sequence_length-1)))  # batch_size x seq_len
            r = tf.matmul(tf.transpose(h_state, [0, 2, 1]),  # (batch,hidden_size,seq_len)
                          tf.reshape(alpha, [-1, max_target_sequence_length-1, 1]))
            r = tf.squeeze(r)
            l_c = tf.tanh(r)  # (batch , HIDDEN_SIZE)
            l_c = tf.nn.dropout(l_c, keep_prob)
            l_c = layer_normalization(l_c)

            x,_,_,_ = encoder(encoder_embed_input,max_target_sequence_length,keep_prob,reuse = tf.AUTO_REUSE)
            l_cx = tf.concat([l_c, x], 1)  # a [batch c_hidden+z_size]

            FC_W = tf.Variable(tf.truncated_normal([c_hidden+vae_hidden, label_size], stddev=0.1))
            # L2
            tf.add_to_collection("losses_c", tf.contrib.layers.l2_regularizer(0.001)(FC_W))
            FC_b = tf.Variable(tf.constant(0., shape=[label_size]))
            pred = tf.nn.xw_plus_b(l_cx, FC_W, FC_b)
            return pred
...
def encoder(encoder_embed_input,max_target_sequence_length,keep_prob=0.5, reuse=False):
    with tf.variable_scope("encoder", reuse=reuse):
        ...
        # Attention
        W = tf.Variable(tf.random_normal([vae_hidden], stddev=0.1))
        outputs = tf.transpose(outputs, [1, 0, 2])
        M = tf.tanh(outputs)  # M = tanh(h_state)  (batch_size, seq_len, HIDDEN_SIZE)

        alpha = tf.nn.softmax(tf.reshape(tf.matmul(tf.reshape(M, [-1, vae_hidden]),
                                                   tf.reshape(W, [-1, 1])),
                                         (-1, max_target_sequence_length-1)))  # batch_size x seq_len
        r = tf.matmul(tf.transpose(outputs, [0, 2, 1]),  # (batch,hidden_size,seq_len)
                      tf.reshape(alpha, [-1, max_target_sequence_length-1, 1]))
        r = tf.squeeze(r)
        x = tf.tanh(r)  # (batch , HIDDEN_SIZE)
        x = tf.nn.dropout(x, keep_prob)

        #new_states = states[-1]
        z_mean = tf.contrib.layers.fully_connected(inputs=x, num_outputs=z_size, activation_fn=None,
                                                   scope="z_mean")
        z_stddev = tf.contrib.layers.fully_connected(inputs=x, num_outputs=z_size, activation_fn=None,
                                                     scope="z_std")
...
def get_cost_l(encoder_embed_input, decoder_embed_input, l_y, decoder_y, target_sequence_length,
               max_target_sequence_length, batch_size,pred, reuse=False):
    x,z,encode_states,latent_loss = encoder(encoder_embed_input,max_target_sequence_length,keep_prob, reuse = tf.AUTO_REUSE)

    #c_state
    l_yz = tf.concat([z, l_y], 1)
    c_state = tf.nn.softplus(tf.matmul(l_yz, weights_de['w_']) + biases_de['b_'])

    # x_c = tf.tanh(x)  # (batch , x_size)
    # x_c = tf.nn.dropout(x_c, keep_prob)
    # x_c = layer_normalization(x_c)
    # pred_vae = tf.matmul(x_c, weights_de['w_c']) + biases_de['b_c']
    # vae_pred_loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=pred_vae, labels=pred))
    ...
    #cost = tf.add(cost,vae_pred_loss)
    return cost
    ...
    for label in range(label_size):
        ...
        if batch_y != l_y:
            wrong_label_cost = get_cost_l(encoder_embed_input, decoder_embed_input, batch_y, vae_y_all[label],
                                          target_sequence_length,
                                          max_target_sequence_length, batch_size,pred, reuse=True)
...
def get_cost_u(u_encoder_embed_input, u_decoder_embed_input):
    prob_y = classifer(u_encoder_embed_input, un_max_target_sequence_length, keep_prob=keep_prob, reuse=True)
    for label in range(label_size):
        y_i = get_onehot(label)
        cost_l = get_cost_l(u_encoder_embed_input, u_decoder_embed_input, [y_i] * un_batch_size, vae_y_u[label],
                            un_target_sequence_length, un_max_target_sequence_length, un_batch_size,prob_y,reuse=True)
...
with tf.name_scope("weight_inital"):
    weights_de = {
        ...
        'w_c': tf.Variable(tf.random_normal([vae_hidden, label_size], mean=0.0, stddev=0.01)),
        'out': tf.Variable(tf.random_normal([2 * c_hidden, label_size]))
    }
    biases_de = {
        'b_': tf.Variable(tf.random_normal([vae_hidden], mean=0.0, stddev=0.01)),
        'b_c': tf.Variable(tf.random_normal([label_size], mean=0.0, stddev=0.01)),
...
# cost=cost_c
#cost = tf.reduce_mean(target_cost_l) + cost_c
...
#-----------------------------------------main---------------------------------------------
def train_model():
    #exclude = ['classifier_1','classifier','Variable','Adam','encoder_3','Adam_1']
    ...
    with tf.Session(config=config) as  sess:
        sess.run(initial)
        saver.restore(sess, './temp/imdb_2500_new_con.pkt')
        ...
        for epoch in range(iter_num):
            ...
            while step < len(new_trainS) // batch_size:  # 2500/25
                ...
                acc_un, ulab, p_y,acc_l, wrong_l_cost, op, pred_batch, c_pred, batch_cost, l_cost, c_cost,u_cost = sess.run(
                    [un_acc,L_ulab,prob_y,l_acc, wrong_cost_l, optimizer, pred, correct_pred, cost, cost_l, cost_c,cost_u
        ...
        #model_path = './temp/imdb_%d_new_con.pkt' % Label_Train_Size

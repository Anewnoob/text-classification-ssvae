...
def embedding_class(features,is_reuse=None):
...
def classifer(encoder_embed_input, target_sequence_length,max_target_sequence_length, keep_prob=0.5, reuse=False):
    with tf.variable_scope("classifier", reuse=reuse):
        encoder_input= embedding(encoder_embed_input,is_reuse=reuse)  # b * s * e
        ...
        fw_lstm_cell = tf.nn.rnn_cell.LSTMCell(c_hidden, forget_bias=1.0,
        ...
        bw_lstm_cell = tf.nn.rnn_cell.LSTMCell(c_hidden, forget_bias=1.0,
                                                        state_is_tuple=True)  # state_is_tuple=True
...
def encoder(encoder_embed_input, y, keep_prob=0.5, reuse=False):
    with tf.variable_scope("encoder", reuse=reuse):
        #encoder_input= embedding(encoder_embed_input, is_reuse=reuse)
        ...
        #y_pos = tf.argmax(y, -1)
        #_,emb_y = embedding_class(features=y_pos,is_reuse=tf.AUTO_REUSE)
        #new_states = tf.concat([states[0], emb_y], 1)
        new_states = states[-1]
...
def decoder(decoder_embed_input, decoder_y, target_length, max_target_length, encode_state, batch_size, keep_prob,
    ...
    with tf.variable_scope("decoder", reuse=reuse):
        ...
        #decoder_input= embedding( decoder_input_, is_reuse=reuse)
...
def get_cost_l(encoder_embed_input, decoder_embed_input, y, decoder_y, target_sequence_length,
               max_target_sequence_length, batch_size, reuse=False):
    encode_states, z_mean, z_stddev = encoder(encoder_embed_input, y, keep_prob, reuse)
    ...
    #l_yz = tf.concat([z, y], 1)
    y_pos = tf.argmax(y, -1)
    _, emb_y = embedding_class(features=y_pos, is_reuse=tf.AUTO_REUSE)  #(b,c)
    #y_state = tf.nn.tanh(tf.matmul(emb_y, weights_de['w_y']) + biases_de['b_y']) #(64,50)
    z_state = tf.concat([z,emb_y],1)  #(64,300)

    #TODO VAE_Y LOSS
    z_c = layer_normalization(tf.nn.dropout(z_state, keep_prob))
    vae_W = tf.Variable(tf.truncated_normal([z_size+embedding_size, label_size], stddev=0.1))
    vae_b = tf.Variable(tf.constant(0., shape=[label_size]))
    vae_pred = tf.nn.xw_plus_b(z_c, vae_W, vae_b)

    #zy_state =  tf.multiply(z,y_state)
    h_state = tf.nn.tanh(tf.matmul(z_state, weights_de['w_']) + biases_de['b_']) #(64,512)

    #decoder_initial_state = LSTMStateTuple(c_state, encode_states[1])
    decoder_initial_state = clstm.LSTMStateTuple(encode_states[0], h_state)
    ...
    return cost,vae_pred

def get_cost_l_all(encoder_embed_input, decoder_embed_input, l_y, decoder_y, target_sequence_length,
                   max_target_sequence_length, batch_size, reuse=False):
    flag = 0
    real_label_cost,vae_pred = get_cost_l(encoder_embed_input, decoder_embed_input, l_y, decoder_y, target_sequence_length,
                                 max_target_sequence_length, batch_size, reuse=False)
    for label in range(label_size):
        y_i = get_onehot(label)
        batch_y = [y_i] * batch_size
        if batch_y != l_y:
            wrong_label_cost,_ = get_cost_l(encoder_embed_input, decoder_embed_input, batch_y, vae_y_all[label],
                                          target_sequence_length,
    ...
    real_cost_vae = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(logits=vae_pred, labels=l_y))
    cost_vae = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(logits=vae_pred_y, labels=l_y))
    #wrong_cost_vae = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(logits=vae_pred_y, labels=vae_pred))
    same_pred = tf.equal(tf.argmax(tf.nn.softmax(vae_pred), 1), tf.argmax(l_y, 1))
    accuracy = tf.reduce_mean(tf.cast(same_pred, tf.float32))

    target_cost = real_label_cost + real_cost_vae + cost_vae
    return tf.reduce_mean(target_cost), real_label_cost,accuracy,wrong_cost_all

def get_cost_u(u_encoder_embed_input, u_decoder_embed_input):
    prob_y = classifer(u_encoder_embed_input,un_target_sequence_length-1,un_max_target_sequence_length-1, keep_prob=1.0, reuse=True)
    for label in range(label_size):
        y_i = get_onehot(label)
        un_batch_y = [y_i] * un_batch_size
        cost_l,un_vae_pred = get_cost_l(u_encoder_embed_input, u_decoder_embed_input, un_batch_y, vae_y_u[label],
                            un_target_sequence_length, un_max_target_sequence_length, un_batch_size, reuse=True)
    ...
    # un_target_y = tf.nn.softmax(prob_y) + un_vae_y  # 0.1 0.3 0.5 0.7 1.0
    ...
    # un_target_y = tf.nn.softmax(prob_y) + un_vae_y * (0.5 / alpha)  # 0.1 0.3 0.5 0.7 1.0
    # un_target_y = tf.nn.softmax(un_target_y)
    # un_target_y = tf.argmax(un_target_y, 1)
    # un_target_y = tf.one_hot(un_target_y, label_size)
    # UN_FC_W = tf.Variable(tf.truncated_normal([label_size, label_size], stddev=0.1))
    # UN_FC_b = tf.Variable(tf.constant(0., shape=[label_size]))
    # un_target_logits = tf.nn.xw_plus_b(un_vae_pred, UN_FC_W, UN_FC_b)+prob_y
    un_target_y = tf.nn.softmax(prob_y) + tf.nn.softmax(un_vae_pred)
    un_target_y = tf.nn.softmax(un_target_y)
    un_cost_1 = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(logits=prob_y, labels=un_target_y))
    un_cost_2 = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(logits=un_vae_pred, labels=un_target_y))
    un_cost_c = un_cost_1+un_cost_2

    prob_y = tf.nn.softmax(prob_y)
    un_vae_y = tf.nn.softmax(un_vae_pred)
    same_pred1 = tf.equal(tf.argmax(prob_y, 1), tf.argmax(un_vae_y, 1))
    same_pred2 = tf.equal(tf.argmax(prob_y, 1), tf.argmax(un_target_y, 1))
    acc1 = tf.reduce_mean(tf.cast(same_pred1, tf.float32))
    acc2 = tf.reduce_mean(tf.cast(same_pred2, tf.float32))
    U = tf.reduce_sum(tf.multiply(L_ulab, un_target_y), 1)
    return U ,L_ulab, prob_y, acc1,acc2, un_cost_c,un_target_y
...
with tf.name_scope("weight_inital"):
    weights_de = {
        'w_': tf.Variable(tf.random_normal([z_size+embedding_size, vae_hidden], mean=0.0, stddev=0.01)),
        #'w_y': tf.Variable(tf.random_normal([embedding_size, z_size], mean=0.0, stddev=0.01)),
        'out': tf.Variable(tf.random_normal([2 * c_hidden, label_size]))
    }
    biases_de = {
        'b_': tf.Variable(tf.random_normal([vae_hidden], mean=0.0, stddev=0.01)),
        #'b_y': tf.Variable(tf.random_normal([z_size], mean=0.0, stddev=0.01)),
...
pred = classifer(l_encoder_embed_input, target_sequence_length-1,max_target_sequence_length - 1, keep_prob=keep_prob)
cost_c = get_cost_c(pred)
tf.add_to_collection("losses_c", cost_c)
cost_c = tf.add_n(tf.get_collection("losses_c"))

target_cost_l, cost_l,l_acc,wrong_cost_all = get_cost_l_all(l_encoder_embed_input, l_decoder_embed_input, l_y, vae_y,
...
cost_u, L_ulab, prob_y, un_acc1,un_acc2,un_cost_c,un_target_y = get_cost_u(u_encoder_embed_input, u_decoder_embed_input)
...
#cost=cost_c + tf.reduce_mean(target_cost_l)
...
def train_model():
    #exclude = ['Variable_6']
    #exclude = ['class_embed','W_class','Adam_1']
    #exclude = ['classifier','bidirectional_rnn','bw','multi_rnn_cell','cell_0','basic_lstm_cell','bias','class_embed','W_class','x_embed','W','Adam_1']
    ...
    with tf.Session(config=config) as  sess:
        sess.run(initial)
        saver.restore(sess, './temp/imdb_new_version_3_2500.pkt')
        ...
        for epoch in range(iter_num):
            ...
            while step < len(new_trainS) // batch_size:  # 2500/25
                ...
                #acc_un, ulab, p_y,c_pred,batch_cost,c_cost,u_cost  #un_acc1,un_acc2,prob_y,un_target_y ,cost_u
                acc1, acc2, y, target_y, op, pred_batch, c_pred, batch_cost, l_cost,c_cost,u_cost = sess.run(
                    [un_acc1, un_acc2, prob_y, un_target_y,optimizer, pred, correct_pred, cost, cost_l,cost_c,cost_u],  # un_acc,L_ulab,prob_y,cost_c,cost_u,pred,correct_pred,
            ...
            #print (wrong_cost)
            print ("acc_un:", acc1,acc2,"p_y", y,"target_y",target_y)
            ...
            # if epoch == T1:
            #     model_path = './temp/imdb_new_version_3_%d.pkt' % Label_Train_Size

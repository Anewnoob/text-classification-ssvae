...
W_dic = tf.get_variable('W', initializer=dic_embeddings, trainable=False,dtype=tf.float32)
...
def get_cost_l(encoder_embed_input, decoder_embed_input, l_y, decoder_y, target_sequence_length,
    ...
    c_state = tf.nn.softplus(tf.matmul(z_state, weights_de['w_']) + biases_de['b_'])


    decoder_initial_state = clstm.LSTMStateTuple(encode_states[0], c_state)
...
def get_cost_l_all(encoder_embed_input, decoder_embed_input, l_y, decoder_y, target_sequence_length,
    ...
    vae_pred_y = tf.nn.tanh(new_wrong_cost)
    ...
    target_cost = real_label_cost + cost_vae - 0.4*wrong_cost_mean
...
def get_cost_u(u_encoder_embed_input, u_decoder_embed_input):
    logits,logits_class = classifer(u_encoder_embed_input,un_target_sequence_length,un_max_target_sequence_length, keep_prob=1.0, reuse=True)
    for label in range(label_size):
        y_i = get_onehot(label)
    ...
    #un_cost_c = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(logits=prob_y, labels=un_vae_y))
...
logits,logits_class = classifer(l_encoder_embed_input, target_sequence_length,max_target_sequence_length, keep_prob=keep_prob)
...
cost = c_alpha * cost_c + tf.reduce_mean(cost_l) + alpha*tf.reduce_mean(cost_u)#+alpha*un_cost_c
...
def train_model():
    ...
    with tf.Session(config=config) as  sess:
        sess.run(initial)
        saver.restore(sess, './temp/imdb_new_version_4_clstmh2_2500.pkt')
        ...
        model_path = './temp/imdb_new_version_4_clstmh2-70_%d.pkt' % Label_Train_Size

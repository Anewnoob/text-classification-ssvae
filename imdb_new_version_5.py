...
def get_cost_l(encoder_embed_input, decoder_embed_input, l_y, decoder_y, target_sequence_length,
               max_target_sequence_length, batch_size,l_c,reuse=False):
...
def get_cost_u(u_encoder_embed_input, u_decoder_embed_input):
    logits,logits_class,un_l_c = classifer(u_encoder_embed_input,un_target_sequence_length,un_max_target_sequence_length, keep_prob=1.0, reuse=True)
    for label in range(label_size):
        y_i = get_onehot(label)
        cost_l = get_cost_l(u_encoder_embed_input, u_decoder_embed_input, [y_i] * un_batch_size, vae_y_u[label],
                            un_target_sequence_length, un_max_target_sequence_length, un_batch_size,un_l_c, reuse=True)
...
logits,logits_class,l_c = classifer(l_encoder_embed_input, target_sequence_length,max_target_sequence_length, keep_prob=keep_prob)
...
def train_model():
    ...
    with tf.Session(config=config) as  sess:
        sess.run(initial)
        #saver.restore(sess, './temp/imdb_new_version_4_clstmh_2500.pkt')
        ...
        model_path = './temp/imdb_new_version_4_clstmh3_%d.pkt' % Label_Train_Size

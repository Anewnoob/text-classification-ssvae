...
def classifer(encoder_embed_input, target_sequence_length,max_target_sequence_length, keep_prob=0.5, reuse=False):
    with tf.variable_scope("classifier", reuse=reuse):
        ...
        logits = tf.nn.xw_plus_b(l_c, FC_W, FC_b)
...
def encoder(encoder_embed_input, y, keep_prob=0.5, reuse=False):
    with tf.variable_scope("encoder", reuse=reuse):
        ...
        new_states = tf.concat([states[-1], emb_y], 1)
...
def decoder(decoder_embed_input, decoder_y, target_length, max_target_length, encode_state, batch_size, keep_prob,G,
    ...
    with tf.variable_scope("decoder", reuse=reuse):
        ...
        decoder_input = tf.concat([decoder_input,decoder_y,G], 2)  # dic_embedding+y(one-hot)+G
...
def get_cost_l(encoder_embed_input, decoder_embed_input, l_y, decoder_y, target_sequence_length,
               max_target_sequence_length, batch_size,l_c,G,reuse=False):
    ...
    z_state = tf.concat([z,l_c],1)  #(64,50+300)
    h_state = tf.nn.softplus(tf.matmul(z_state, weights_de['w_']) + biases_de['b_'])  #(64*64)
    #decoder_initial_state = clstm.LSTMStateTuple(h_state, encode_states[1])
    ...
    for label in range(label_size):
        ...
        if batch_y != l_y:
            wrong_label_cost = get_cost_l(encoder_embed_input, decoder_embed_input, batch_y, vae_y_all[label],
                                          target_sequence_length,
                                          max_target_sequence_length, batch_size,l_c,G,reuse=True)
...
def get_cost_u(u_encoder_embed_input, u_decoder_embed_input):
    logits,logits_class,un_l_c,G = classifer(u_encoder_embed_input,un_target_sequence_length,un_max_target_sequence_length, keep_prob=1.0, reuse=True)
    ...
    for label in range(label_size):
        ...
        cost_l = get_cost_l(u_encoder_embed_input, u_decoder_embed_input, [y_i] * un_batch_size, vae_y_u[label],
                            un_target_sequence_length, un_max_target_sequence_length, un_batch_size,un_l_c,G,reuse=True)
    ...
    un_target_y = tf.nn.softmax(logits) + un_vae_y  # 0.1 0.3 0.5 0.7 1.0
    un_target_y = tf.nn.softmax(un_target_y)
    un_cost_c = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(logits=logits, labels=un_vae_y))
    ...
    accuracy = tf.reduce_mean(tf.cast(same_pred, tf.float32))
    U = tf.reduce_sum(tf.multiply(L_ulab, prob_y), 1)# + class_loss
    un_label_cost = tf.reduce_mean(U) + class_loss + un_cost_c
...
with tf.name_scope("weight_inital"):
    weights_de = {
        'w_': tf.Variable(tf.random_normal([z_size+c_hidden, vae_hidden], mean=0.0, stddev=0.01)),
...
logits,logits_class,l_c,G = classifer(l_encoder_embed_input, target_sequence_length,max_target_sequence_length, keep_prob=keep_prob)
...
cost = c_alpha * cost_c + tf.reduce_mean(target_cost_l) + alpha*cost_u
...
#-----------------------------------------main---------------------------------------------
def train_model():
    #exclude = ['classifier_1', 'Variable', 'Adam']
    ...
    with tf.Session(config=config) as  sess:
        sess.run(initial)
        #saver.restore(sess, './temp/imdb_new_version_4_clstmh7_2500.pkt')
        ...
        for epoch in range(iter_num):
            ...
            while step < len(new_trainS) // batch_size:  # 2500/25
                ...
                acc_un, ulab, p_y,acc_l,op, pred_batch, c_pred, batch_cost, l_cost, c_cost,u_cost = sess.run(
                    [un_acc,L_ulab,prob_y,l_acc,optimizer, pred, correct_pred, cost, cost_l, cost_c,cost_u
                     ],  # un_acc,L_ulab,prob_y,cost_c,cost_u,pred,correct_pred,
                    feed_dict={vae_y: decode_y, vae_y_all: decode_y_all, vae_y_u: decode_uy,
                               l_encoder_embed_input: input_batch, l_decoder_embed_input: input_batch,
                               l_y: batch_y,
                               u_decoder_embed_input: un_input_batch, u_encoder_embed_input: un_input_batch,
                ...
                # print logit.shape
                if (step % 40 == 0 and step is not 0):
                    print ('min_kl_epoch', min_kl_epoch)
                    print (
                    'TRAIN LOSS', train_cost, 'LABEL COST', label_cost, 'Unlabel Cost', unlabel_cost, 'Classifier Cost',
                    classifier_cost)
                    print ("\n")
                loss = np.mean(batch_cost,dtype = np.float32)
                lbatch_cost = np.mean(l_cost,dtype = np.float32)
                ubatch_cost = np.mean(u_cost,dtype = np.float32)
                cbatch_cost = np.mean(c_cost,dtype = np.float32)  # *alpha_epoch
        ...
        model_path = './temp/imdb_new_version_4_clstmh7_%d.pkt' % Label_Train_Size
...
def test_model(sess, testS, testL, epoch):
    ...
    while step < len(testS) // batch_size:  #
        ...
        f_pred, c_pred, pred_batch = sess.run([correct_pred_f, correct_pred, pred],
                                              feed_dict={l_encoder_embed_input: input_batch, l_y: batch_y,

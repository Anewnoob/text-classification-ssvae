...
def encoder(encoder_embed_input, l_y,keep_prob=0.5, reuse=False):
    with tf.variable_scope("encoder", reuse=reuse):
        ...
        new_states = tf.concat([states[-1],l_y], 1)
...
def get_cost_l(encoder_embed_input, decoder_embed_input, l_y, decoder_y, target_sequence_length,
               max_target_sequence_length, batch_size,l_a,G,reuse=False):
    encode_states, z_mean, z_stddev = encoder(encoder_embed_input, l_y, keep_prob, reuse=reuse)
    ...
    z_state = tf.concat([l_y,z,l_a],1)  #(64,100+300)
...
def get_cost_u(u_encoder_embed_input, u_decoder_embed_input):
    ...
    for label in range(label_size):
        ...
        cost_l = get_cost_l(u_encoder_embed_input, u_decoder_embed_input, [y_i] * un_batch_size, vae_y_u[label],
                            un_target_sequence_length, un_max_target_sequence_length, un_batch_size,un_l_a,G,reuse=True)
...
with tf.name_scope("weight_inital"):
    weights_de = {
        'w_': tf.Variable(tf.random_normal([z_size+label_size+c_hidden, vae_hidden], mean=0.0, stddev=0.01)),
...
cost_l = get_cost_l(l_encoder_embed_input, l_decoder_embed_input, l_y, vae_y,target_sequence_length, max_target_sequence_length,
                    batch_size,l_a,G)
...
def train_model():
    ...
    with tf.Session(config=config) as  sess:
        ...
        for epoch in range(iter_num):
            ...
            if epoch == 14:
                model_path = './temp/imdb_new_version_8_dic_sclstm_%d.pkt' % Label_Train_Size

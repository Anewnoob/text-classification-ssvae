...
def classifier(encoder_embed_input, target_sequence_length,max_target_sequence_length, keep_prob=0.5, reuse=False):
    with tf.variable_scope("classifier", reuse=reuse):
        ...
        return logits,logits_class,l_a,G


def encoder(encoder_embed_input, y, logits,logits_class,keep_prob=0.5, reuse=False):
    with tf.variable_scope("encoder", reuse=reuse):
        ...
        new_states = tf.concat([states[-1],logits], 1)
...
def get_cost_c(logits,logits_class):  # compute classifier cost
    cost = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(logits=logits, labels=l_y)) + tf.reduce_mean(
...
def get_cost_l(encoder_embed_input, decoder_embed_input, l_y, decoder_y, target_sequence_length,
               max_target_sequence_length, batch_size,logits,logits_class,l_a,G,reuse=False):
    encode_outputs, encode_states, z_mean, z_stddev = encoder(encoder_embed_input, l_y,logits,logits_class, keep_prob, reuse)
    ...
    z_state_1 = tf.concat([z,l_a],1)  #(64,100+300)
    h_state = tf.nn.tanh(tf.matmul(z_state_1, weights_de['w_']) + biases_de['b_'])  #(64*64)
    z_state_2 = tf.concat([encode_states[0],l_a],1)
    c_state = tf.nn.tanh(tf.matmul(z_state_2, weights_de['w_c']) + biases_de['b_c'])
    decoder_initial_state = clstm.LSTMStateTuple(c_state, h_state)
    ...
    for label in range(label_size):
        ...
        if batch_y != l_y:
            wrong_label_cost = get_cost_l(encoder_embed_input, decoder_embed_input, batch_y, vae_y_all[label],
                                          target_sequence_length,
                                          max_target_sequence_length, batch_size,logits,logits_class,l_a,G,reuse=True)
...
def get_cost_u(u_encoder_embed_input, u_decoder_embed_input):
    logits,logits_class,un_l_a,G = classifier(u_encoder_embed_input,un_target_sequence_length,un_max_target_sequence_length, keep_prob=keep_prob, reuse=True)
    class_loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(labels=class_y, logits=logits_class))
    #loss_entropy = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=logits, logits=logits))
    for label in range(label_size):
        y_i = get_onehot(label)
        cost_l = get_cost_l(u_encoder_embed_input, u_decoder_embed_input, [y_i] * un_batch_size, vae_y_u[label],
                            un_target_sequence_length, un_max_target_sequence_length, un_batch_size,logits,logits_class,un_l_a,G,reuse=True)
...
logits,logits_class,l_a,G = classifier(l_encoder_embed_input, target_sequence_length,max_target_sequence_length, keep_prob=keep_prob)
...
target_cost_l, cost_l, wrong_cost_l, l_acc = get_cost_l_all(l_encoder_embed_input, l_decoder_embed_input, l_y, vae_y,
                                                             ...
                                                             batch_size,logits,logits_class,l_a,G)
...
cost = c_alpha*cost_c + target_cost_l + alpha*cost_u
...
def train_model():
    ...
    with tf.Session(config=config) as  sess:
        sess.run(initial)
        #saver.restore(sess, './temp/imdb_new_version_9_2500.pkt')
        ...
        model_path = './temp/imdb_new_version_9_%d.pkt' % Label_Train_Size

...
if not os.path.exists(target_pkl_path):
    ...
    train_len = len(new_trainS)
    test_len  = len(new_testS)
    unlabel_len = len(new_unlabelS)
    print ("load data pkl successful!")
    print ("train:",train_len,"test:",test_len,"unlabel:",unlabel_len)
...
def get_cost_l(encoder_embed_input, decoder_embed_input, y, decoder_y, target_sequence_length,
    ...
    c_state = tf.nn.tanh(tf.matmul(z_state, weights_de['w_']) + biases_de['b_']) #(64,512)
...
def get_cost_l_all(encoder_embed_input, decoder_embed_input, l_y, decoder_y, target_sequence_length,
    ...
    vae_pred = tf.nn.softmax(vae_pred)
    same_pred1 = tf.equal(tf.argmax(vae_pred,1), tf.argmax(l_y, 1))
    acc1 = tf.reduce_mean(tf.cast(same_pred1, tf.float32))

    same_pred2 = tf.equal(tf.argmax(vae_y,1), tf.argmax(l_y, 1))
    acc2 = tf.reduce_mean(tf.cast(same_pred2, tf.float32))

    target_cost = real_label_cost + cost_vae + real_cost_vae
    return tf.reduce_mean(target_cost),real_label_cost,acc1,acc2,wrong_cost_all
...
target_cost_l,cost_l,l_acc1,l_acc2,wrong_vae_cost = get_cost_l_all(l_encoder_embed_input, l_decoder_embed_input, l_y, vae_y,
...
def train_model():
    ...
    with tf.Session(config=config) as  sess:
        sess.run(initial)
        #saver.restore(sess, './temp/imdb_new_pre_2500.pkt')
        ...
        for epoch in range(iter_num):
            ...
            while step < len(new_trainS) // batch_size:  # 2500/25
                ...
                acc1,acc2,vae_all_cost,op, batch_cost, l_cost = sess.run(
                    [l_acc1,l_acc2,wrong_vae_cost,optimizer, cost, cost_l],  # un_acc,L_ulab,prob_y,cost_c,cost_u,pred,correct_pred,
        ...
        model_path = './temp/imdb_new_version_vae_%d.pkt' % Label_Train_Size

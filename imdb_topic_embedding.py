...
if not os.path.exists(target_pkl_path):
    ...
    dic_embeddings, new_unlabelS, new_testS, new_trainS, int_to_vocab, vocab_to_int, new_trainL, new_testL, label_list = dump_data(data_path,embedding_size,Label_Train_Size,Unlabel_Train_Size)
else:
    ...
    #topic
    # neg_topic_index = vocab_to_int[class_name[0]]
    # pos_topic_index = vocab_to_int[class_name[1]]
...
def decoder(decoder_embed_input,l_y,decoder_y, target_length, max_target_length, encode_state, batch_size, keep_prob,
            reuse=False):
...
def get_cost_l(encoder_embed_input, decoder_embed_input, l_y, decoder_y, target_sequence_length,
    ...
    decoder_output, predicting_logits, training_logits, masks, target = decoder(decoder_embed_input,l_y,decoder_y,
...
def get_cost_l_all(encoder_embed_input, decoder_embed_input, l_y, decoder_y, target_sequence_length,
    ...
    target_cost = real_label_cost + cost_vae - 0.05*wrong_cost_mean

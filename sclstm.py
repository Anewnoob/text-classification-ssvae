...
class BasicLSTMCell(RNNCell):
  ...
  def __init__(self, num_units,label_size = None,embedding_size= None,context=None,l_y = None,W_class = None,
               forget_bias=1.0,
  ...
  def call(self, inputs, state):
    ...
    #embedding  label_y
    #c = array_ops.split(c, num_or_size_splits=2, axis=1)
    ...
    #G = array_ops.slice(inputs,[0,self._embedding_size+self._label_size],[-1,self._label_size])
    ...
    # y_pos = tf.argmax(label, -1)
    # features = tf.to_int32(y_pos)
    #label = tf.nn.embedding_lookup(self._W_class, features)
    ...
    # print(c.shape,i.shape,j.shape,f.shape,o.shape)
    # print(lstm_label_matrix.shape)

    new_c = (c * sigmoid(f + self._forget_bias) + sigmoid(i) * self._activation(j) + self._activation(lstm_label_matrix))
    new_h = self._activation(new_c) * sigmoid(o) #+ self._activation(G_matrix)
...
class LSTMCell(RNNCell):
  ...
  def __init__(self, num_units,label_size,embedding_size,
  ...
  def call(self, inputs, state):
    ...
    input_list = ops.convert_n_to_tensor(input_list)
    #label_list = ops.convert_n_to_tensor([self._label])
    ...
    with vs.variable_scope(scope, initializer=self._initializer) as unit_scope:
      ...
      lstm_matrix = _linear([input_list[0],m_prev], 4 * self._num_units, bias=True)
      lstm_label_matrix = _linear(input_list[1], self._num_units,_WEIGHTS_VARIABLE_NAME="weight_y",_BIAS_VARIABLE_NAME="bias_y",
      ...
      if self._use_peepholes:
        c = (sigmoid(f + self._forget_bias + w_f_diag * c_prev) * c_prev +
             sigmoid(i + w_i_diag * c_prev) * self._activation(j) + self._activation(lstm_label_matrix))
        #add label
      else:
        c = (sigmoid(f + self._forget_bias) * c_prev + sigmoid(i) *
             self._activation(j)+self._activation(lstm_label_matrix))
...
def _linear(args,
  ...
  with vs.variable_scope(scope) as outer_scope:
    ...
    if len(args) == 1:
      ...
      #print(array_ops.concat(args, 1),weights)

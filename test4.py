...
def encoder(encoder_embed_input, l_y, logits,l_a,keep_prob=0.5, reuse=False):
    with tf.variable_scope("encoder", reuse=reuse):
        ...
        new_feature = tf.concat([states[-1],l_a],1)

        f = tf.contrib.layers.fully_connected(inputs=new_feature, num_outputs=f_size, activation_fn = tf.nn.tanh,
                                                   scope="f")
        ...
        o_stddev = tf.contrib.layers.fully_connected(inputs=new_states, num_outputs=z_size, activation_fn=None,
                                                     scope="z_std")
        return states, o_mean, o_stddev,f
...
def get_cost_l(encoder_embed_input, decoder_embed_input, l_y, decoder_y, target_sequence_length,
    ...
    encode_states, z_mean, z_stddev,f = encoder(encoder_embed_input, l_y,logits,l_a, keep_prob, reuse)
    ...
    z_state = tf.concat([logits,z,f],1)  #(64,100+300)
...
def get_cost_u(u_encoder_embed_input, u_decoder_embed_input):
    logits,logits_class,un_l_a,G = classifier(u_encoder_embed_input,un_target_sequence_length-1,un_max_target_sequence_length-1, keep_prob=keep_prob, reuse=True)
    ...
    for label in range(label_size):
        ...
        cost_l = get_cost_l(u_encoder_embed_input, u_decoder_embed_input, [y_i] * un_batch_size, vae_y_u[label],
                            un_target_sequence_length, un_max_target_sequence_length, un_batch_size,logits,un_l_a,G,reuse=True)
...
with tf.name_scope("weight_inital"):
    weights_de = {
        'w_': tf.Variable(tf.random_normal([z_size+label_size+f_size, vae_hidden], mean=0.0, stddev=0.01)),
...
logits,logits_class,l_a,G = classifier(l_encoder_embed_input, target_sequence_length-1,max_target_sequence_length-1, keep_prob=keep_prob)
...
def train_model():
    ...
    with tf.Session(config=config) as  sess:
        ...
        for epoch in range(iter_num):
            ...
            if epoch == 19:
                model_path = './temp/test4_dic_nosclstm_%d.pkt' % Label_Train_Size

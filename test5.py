...
def encoder(encoder_embed_input, l_y, logits,l_a,keep_prob=0.5, reuse=False):
    with tf.variable_scope("encoder", reuse=reuse):
        ...
        x_f = tf.contrib.layers.fully_connected(inputs=states[-1], num_outputs=f_size, activation_fn = tf.nn.tanh,
                                                   scope="x_f")

        new_states = tf.concat([l_a,x_f,l_y], 1)

        f_mean = tf.contrib.layers.fully_connected(inputs=new_states, num_outputs=f_size, activation_fn = tf.nn.tanh,
                                                   scope="f_mean")

        f_stddev = tf.contrib.layers.fully_connected(inputs=new_states, num_outputs=f_size, activation_fn = tf.nn.tanh,
                                                   scope="f_std")

        samples = tf.random_normal(tf.shape(f_stddev))
        f = f_mean + tf.exp(f_stddev * 0.5) * samples

        z_mean = tf.contrib.layers.fully_connected(inputs=new_states, num_outputs=z_size, activation_fn=tf.nn.tanh,
                                                   scope="z_mean")
        z_stddev = tf.contrib.layers.fully_connected(inputs=new_states, num_outputs=z_size, activation_fn=tf.nn.tanh,
                                                     scope="z_std")

        samples = tf.random_normal(tf.shape(z_stddev))
        z = z_mean + tf.exp(z_stddev * 0.5) * samples
        z_latent_loss = 0.5 * tf.reduce_sum(tf.exp(z_stddev) - 1. - z_stddev + tf.square(z_mean), 1)
        f_latent_loss = 0.5 * tf.reduce_sum(tf.exp(z_stddev) - 1. - z_stddev + tf.square(z_mean), 1)

        latent_loss = z_latent_loss + f_latent_loss

        return states, z , f ,latent_loss
...
#------------------------------------Get loss------------------------------------------
def get_cost_c(logits,logits_class,vae_logits):  # compute classifier cost
    cost = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(logits=logits, labels=l_y)) + 0.5*tf.reduce_mean(
            ...
            #tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(logits=vae_logits, labels=l_y))
    return cost
...
def get_cost_l(encoder_embed_input, decoder_embed_input, l_y, target_sequence_length,
    ...
    encode_states, z , f ,latent_loss = encoder(encoder_embed_input,l_y,logits,l_a, keep_prob, reuse)

    f = tf.nn.dropout(f,keep_prob= keep_prob)
    f_y = tf.contrib.layers.fully_connected(inputs=f, num_outputs=label_size, activation_fn=tf.nn.softmax)
    #confidence = tf.nn.softmax_cross_entropy_with_logits_v2(logits=f_y, labels=l_y)
    #confidence = tf.clip_by_value(confidence, 1e-10, 1.0)
    #R  = tf.reduce_mean(tf.multiply(f_y, l_y), 1)

    z_state = tf.concat([z,f,l_y],1)  #(64,100+300)
    ...
    #encropy_loss = -tf.reduce_sum(decoder_input * tf.log(training_logits) + (1 - decoder_input) * tf.log(1 - training_logits), 2)
    #encropy_loss = tf.nn.softmax_cross_entropy_with_logits_v2(labels=decoder_input, logits=training_logits)
...
def get_cost_u(u_encoder_embed_input, u_decoder_embed_input):
    ...
    u_cost_l,u_prob_y = get_cost_l(u_encoder_embed_input, u_decoder_embed_input, prob_y,
    ...
    #confidence_entropy = tf.reduce_mean(prob_y * tf.log(tf.clip_by_value(confidence, 1e-10, 1.0)))
    #cross_entropy = tf.reduce_mean(prob_y * tf.log(prob_y) + (1 - prob_y) * tf.log(1 - prob_y))

    # prob_entropy = tf.reduce_mean(prob_y * tf.log(prob_y))
    #
    # u_prob_y = tf.nn.softmax(u_prob_y)
    #
    # cross_entropy = tf.reduce_mean(prob_y * tf.log(u_prob_y),1)
    # confidence = tf.clip_by_value(cross_entropy,-5.0, 0)
    # confidence = tf.expand_dims(confidence, 1)
    # V = tf.concat([confidence, confidence], 1)
    #
    # R = tf.reduce_sum(V * prob_y)

    un_label_cost = tf.reduce_mean(u_cost_l) #- prob_entropy #- R
...
cost_l,vae_logits = get_cost_l(l_encoder_embed_input, l_decoder_embed_input, l_y,target_sequence_length, max_target_sequence_length,
                    batch_size,logits,l_a)

cost_u, prob_y = get_cost_u(u_encoder_embed_input, u_decoder_embed_input)


cost_c = get_cost_c(logits,logits_class,vae_logits)
...
correct_pred_f = tf.cast(correct_pred, tf.float32)
accuracy = tf.reduce_mean(tf.cast(correct_pred, tf.float32))

correct_pred_f = tf.clip_by_value(correct_pred_f,0, 1.0)
correct_pred_f = tf.expand_dims(correct_pred_f, 1)
V_L = tf.concat([correct_pred_f,correct_pred_f],1)
R_L =  tf.reduce_sum(V_L * pred)



cost = c_alpha*cost_c #+ tf.reduce_mean(cost_l) + alpha*cost_u #- R_L
    ...
    with tf.Session(config=config) as  sess:
        ...
        for epoch in range(iter_num):
            ...
            while step < len(new_trainS) // batch_size:  # 2500/25
                ...
                L_R,L_V,p_y,op, pred_batch, c_pred, batch_cost, l_cost, c_cost,u_cost = sess.run(
                    [R_L,V_L,prob_y,optimizer, pred, correct_pred, cost, cost_l, cost_c,cost_u

...
def encoder(encoder_embed_input,l_y,l_a,keep_prob=0.5, reuse=False):
    with tf.variable_scope("encoder", reuse=reuse):
        ...
        new_states = tf.concat([l_a,x_f], 1)
        f_mean = tf.contrib.layers.fully_connected(inputs=new_states, num_outputs=f_size, activation_fn = None,
                                                   scope="f_mean")

        f_stddev = tf.contrib.layers.fully_connected(inputs=new_states, num_outputs=f_size, activation_fn = None,
                                                   scope="f_std")
        ...
        z_latent_loss = 0.5 * tf.reduce_mean(tf.square(z_mean) + tf.square(z_stddev) - tf.log(1e-8 + tf.square(z_stddev)) - 1)
        f_latent_loss = 0.5 * tf.reduce_mean(tf.square(f_mean) + tf.square(f_stddev) - tf.log(1e-8 + tf.square(f_stddev)) - 1)
        #latent_loss = 0.5 * tf.reduce_sum(tf.exp(z_stddev) - 1. - z_stddev + tf.square(z_mean), 1)
        #density =  -0.5 * (tf.log(2 * np.pi) + (tf.square(z_mean) + tf.exp(1e-8 + z_stddev)))
...
#------------------------------------Get loss------------------------------------------
def get_cost_c(logits,logits_class,f_y):  # compute classifier cost
    cost = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(logits=logits, labels=l_y)) + 0.5*tf.reduce_mean(
        tf.nn.softmax_cross_entropy_with_logits_v2(labels=class_y, logits=logits_class))\
           +tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(logits=f_y, labels=l_y))
    return cost


def get_cost_l(encoder_embed_input, decoder_embed_input, l_y, target_sequence_length,
               max_target_sequence_length, batch_size,l_a,reuse=False):
    states, z, f, latent_loss = encoder(encoder_embed_input, l_y,l_a, keep_prob, reuse)

    f_y = tf.contrib.layers.fully_connected(inputs=f, num_outputs=label_size, activation_fn=tf.nn.softmax)


    z_state = tf.concat([l_y,z,f_y],1)  #(64,100+300)
    c_state = tf.nn.tanh(tf.matmul(z_state, weights_de['w_']) + biases_de['b_'])  #(64*64)


    decoder_initial_state = LSTMStateTuple(c_state, states[1])
    ...
    s_loss = 0.5 * tf.square(training_logits - decoder_input)  # batch,len,embeding_size
    mask_loss = tf.reduce_sum(tf.transpose(s_loss, [2, 0, 1]), 0)  # mask_loss (bacth_size,max_len_seq)
    encropy_loss = tf.reduce_mean(tf.multiply(mask_loss, masks))  # 还原句子长度 其余位置都是0　　multiply　点乘
...
def get_cost_u(u_encoder_embed_input, u_decoder_embed_input):
    ...
    u_cost_l,un_f_y = get_cost_l(u_encoder_embed_input, u_decoder_embed_input, prob_y,
                            un_target_sequence_length, un_max_target_sequence_length, un_batch_size,un_l_a,reuse=True)


    vae_prob_y = tf.nn.softmax(un_f_y)

    cross_entropy = tf.reduce_mean(prob_y * tf.log(prob_y)+(1-prob_y) * (tf.log(1-prob_y)))
    un_label_cost = u_cost_l + class_loss - cross_entropy
...
with tf.name_scope("weight_inital"):
    weights_de = {
        'w_': tf.Variable(tf.random_normal([z_size+2*label_size, vae_hidden], mean=0.0, stddev=0.01)),
...
cost_l,f_y = get_cost_l(l_encoder_embed_input, l_decoder_embed_input, l_y,target_sequence_length, max_target_sequence_length,
                    batch_size,l_a)

cost_u, prob_y = get_cost_u(u_encoder_embed_input, u_decoder_embed_input)


cost_c = get_cost_c(logits,logits_class,f_y)

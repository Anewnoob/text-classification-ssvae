...
# new_trainS = new_trainS[:len(new_trainS)]
# new_trainL = new_trainL[:len(new_trainL)]
...
W_dic = tf.get_variable('W_dic', initializer=dic_embeddings, trainable=True, dtype=tf.float32)
...
def classifier(encoder_embed_input, target_sequence_length,max_target_sequence_length,l_a,keep_prob=0.5, reuse=False):
    with tf.variable_scope("classifier", reuse=reuse):
        encoder_input = tf.nn.embedding_lookup(W_dic, encoder_embed_input)
        ...
        # l_c = tf.concat([l_c,fw_states[0][1],bw_states[0][1]], 1)
        ...
        l_cfb = tf.concat([l_c,l_fb,l_a], 1)

        FC_W = tf.Variable(tf.truncated_normal([2 * c_hidden+a_size, label_size], stddev=0.1))
...
def encoder(l_encoder_embed_input,keep_prob=0.5,reuse=False,is_training = False):
    with tf.variable_scope("encoder", reuse=reuse):
        ...
        l_x = layer_normalization(l_x)
        l_x_to_a = tf.contrib.layers.fully_connected(inputs=l_x, num_outputs=a_size, activation_fn=tf.nn.tanh,scope="x-to-a")
        #l_x_to_a = batch_norm(l_x_to_a, is_training=is_training, scope="BN1")
        a_mean = tf.contrib.layers.fully_connected(inputs=l_x_to_a, num_outputs=a_size, activation_fn=None,scope="a_mean")
        #a_mean = batch_norm(a_mean, is_training=is_training, scope="BN2")
        a_stddev = tf.contrib.layers.fully_connected(inputs=l_x_to_a, num_outputs=a_size, activation_fn=None,scope="a_std")
        #a_stddev = batch_norm(a_stddev, is_training=is_training, scope="BN3")
        samples = tf.random_normal(tf.shape(a_stddev))
        l_a = a_mean + tf.exp(a_stddev * 0.5) * samples
        return l_a,l_x,states, a_mean, a_stddev


def decoder(decoder_embed_input,decoder_y,target_length,max_target_length,l_x,l_a,l_y,states,batch_size,keep_prob,reuse=False,is_training = False):
    with tf.variable_scope("decoder", reuse=reuse):
        l_axy=tf.concat([l_x,l_a,l_y],1)
        l_axy = layer_normalization(l_axy)
        z_mean = tf.contrib.layers.fully_connected(inputs=l_axy, num_outputs=z_size, activation_fn=None,scope="z_mean")
        #z_mean = batch_norm(z_mean, is_training=is_training, scope="BN4")
        z_stddev = tf.contrib.layers.fully_connected(inputs=l_axy, num_outputs=z_size, activation_fn=None,scope="z_std")
        #z_stddev = batch_norm(z_stddev, is_training=is_training, scope="BN5")
        samples = tf.random_normal(tf.shape(z_stddev))
        l_z = z_mean + tf.exp(z_stddev * 0.5) * samples

        l_yz = tf.concat([l_y,l_z], 1)
        l_yz = layer_normalization(l_yz)
        u_mean = tf.contrib.layers.fully_connected(inputs=l_yz, num_outputs=u_size, activation_fn=None, scope="u_mean")
        #u_mean = batch_norm(u_mean, is_training=is_training, scope="BN6")
        u_stddev = tf.contrib.layers.fully_connected(inputs=l_yz, num_outputs=u_size, activation_fn=None, scope="u_std")
        #u_stddev = batch_norm(u_stddev, is_training=is_training, scope="BN7")
        samples = tf.random_normal(tf.shape(u_stddev))
        l_u = u_mean + tf.exp(u_stddev * 0.5) * samples

        l_yzu = tf.concat([l_yz, l_u], 1)
        l_yzu = layer_normalization(l_yzu)
        h_states = tf.nn.tanh(tf.matmul(l_yzu, weights_de['w_']) + biases_de['b_'])
        #h_states = batch_norm(h_states, is_training=is_training, scope="BN8")
        decoder_initial_state = clstm.LSTMStateTuple(h_states, states[1])
        ...
        return training_logits,masks,u_mean,u_stddev,z_mean,z_stddev
...
def get_cost_l(decoder_embed_input,training_logits, masks,a_mean, a_stddev, z_mean, z_stddev,u_mean,u_stddev):
...
def get_cost_u(u_encoder_embed_input, u_decoder_embed_input):
    l_a,l_x,states, a_mean, a_stddev = encoder(u_encoder_embed_input, keep_prob,reuse=True,is_training=training)
    logits,logits_class = classifier(u_encoder_embed_input,un_target_sequence_length-1,un_max_target_sequence_length-1,l_a,keep_prob=keep_prob, reuse=True)
    ...
    for label in range(label_size):
        ...
        training_logits, masks, u_mean, u_stddev,z_mean,z_stddev = decoder(u_decoder_embed_input, vae_y_u[label], un_target_sequence_length,
                                                           un_max_target_sequence_length,l_x,l_a, [y_i]*un_batch_size, states,un_batch_size,keep_prob,reuse=True,is_training=training)
        cost_l = get_cost_l(u_decoder_embed_input, training_logits, masks, a_mean, a_stddev, z_mean, z_stddev, u_mean,
                            u_stddev)
    ...
    U = tf.reduce_sum(tf.multiply(L_ulab, prob_y), 1)
    un_label_cost = tf.reduce_mean(U) + class_loss + cross_entropy
    return un_label_cost, L_ulab, prob_y, accuracy
...
with tf.name_scope("weight_inital"):
    weights_de = {
        'w_': tf.Variable(tf.random_normal([z_size+label_size+a_size, vae_hidden], mean=0.0, stddev=0.01)),
        'w_c': tf.Variable(tf.random_normal([vae_hidden+c_hidden, vae_hidden], mean=0.0, stddev=0.01)),
        'out': tf.Variable(tf.random_normal([2 * c_hidden, label_size]))
    }
    biases_de = {
        'b_': tf.Variable(tf.random_normal([vae_hidden], mean=0.0, stddev=0.01)),
        'b_c': tf.Variable(tf.random_normal([vae_hidden], mean=0.0, stddev=0.01)),
...
input_x = tf.placeholder(dtype=tf.int32,name='input_x')
l_y = tf.placeholder(dtype=tf.float32, shape=[batch_size, label_size],name='l_y')
vae_y = tf.placeholder("float", [batch_size, None, label_size],name='vae_y')  # vae_y
vae_y_all = tf.placeholder("float", [label_size, batch_size, None, label_size],name='vae_y_all')
vae_y_u = tf.placeholder("float", [label_size, un_batch_size, None, label_size],name='vae_y_u')
...
l_decoder_embed_input = tf.placeholder(dtype=tf.int32, shape=[batch_size, None],name='l_decoder')
l_encoder_embed_input = tf.placeholder(dtype=tf.int32, shape=[batch_size, None],name='l_encoder')
u_encoder_embed_input = tf.placeholder(dtype=tf.int32, shape=[un_batch_size, None],name='u_encoder')
u_decoder_embed_input = tf.placeholder(dtype=tf.int32, shape=[un_batch_size, None],name='u_decoder')
latentscale_iter = tf.placeholder(dtype=tf.float32,name='latentscale_iter')
class_y = tf.constant(name='class_y', shape=[label_size, label_size], dtype=tf.float32,value=np.identity(label_size))


l_a,l_x, states, a_mean, a_stddev  = encoder(l_encoder_embed_input,keep_prob = keep_prob,is_training=training)
training_logits, masks,u_mean,u_stddev,z_mean,z_stddev = decoder(l_decoder_embed_input,vae_y,target_sequence_length,max_target_sequence_length,l_x,l_a,l_y,states,batch_size,keep_prob = keep_prob,is_training=training)
logits,logits_class = classifier(l_encoder_embed_input,target_sequence_length-1,max_target_sequence_length-1,l_a,keep_prob=keep_prob)
cost_c = get_cost_c(logits,logits_class)
tf.add_to_collection("losses_c", cost_c)
cost_c = tf.add_n(tf.get_collection("losses_c"))

cost_l = get_cost_l(l_decoder_embed_input,training_logits, masks,a_mean, a_stddev, z_mean, z_stddev,u_mean,u_stddev)

cost_u, L_ulab, prob_y, un_acc = get_cost_u(u_encoder_embed_input, u_decoder_embed_input)
...
def train_model():
    ...
    with tf.Session(config=config) as  sess:
        sess.run(initial)
        # saver.restore(sess, './temp/imdb_new_version_8_5_2500.pkt')
        ...
        for epoch in range(iter_num):
            ...
            while step < len(new_trainS) // batch_size:  # 2500/25
                ...
                acc_un, ulab, p_y, op, pred_batch, c_pred, batch_cost, l_cost, c_cost, u_cost = sess.run(
                    [un_acc, L_ulab, prob_y, optimizer, pred, correct_pred, cost, cost_l, cost_c, cost_u
                ...
                if (step % 100 == 0 and step is not 0):
                    ...
                    print(
                        'TRAIN LOSS', train_cost, 'LABEL COST', label_cost, 'Unlabel Cost', unlabel_cost,
                        'Classifier Cost',
                        classifier_cost)
                    print("\n")
            ...
            print("acc_un:", acc_un, "p_y", p_y, "u_lab", ulab)
            ...
            if epoch == 19:
                model_path = './temp/imdb_new_version_8_dic_%d.pkt' % Label_Train_Size

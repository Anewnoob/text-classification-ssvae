...
if not os.path.exists(target_pkl_path):
    ...
    dic_embeddings, new_unlabelS, new_testS, new_trainS, int_to_vocab, vocab_to_int, new_trainL, new_testL, label_list = dump_data(
        data_path, embedding_size, Label_Train_Size)
else:
...
new_trainS = new_trainS[:Label_Train_Size]
new_trainL = new_trainL[:Label_Train_Size]
...
def classifier(encoder_embed_input, target_sequence_length, max_target_sequence_length, keep_prob=0.5, reuse=False):
    with tf.variable_scope("classifier", reuse=reuse):
        ...
        return logits, logits_class, l_c, G


def encoder(encoder_embed_input,logits,l_a, keep_prob=0.5, reuse=False):
    with tf.variable_scope("encoder", reuse=reuse):
        encoder_input = tf.nn.embedding_lookup(vae_dic, encoder_embed_input)
        input_ = tf.transpose(encoder_input, [1, 0, 2])
        ...
        new_states = tf.concat([l_a, states[-1]], 1)

        z_mean = tf.contrib.layers.fully_connected(inputs=new_states, num_outputs=z_size, activation_fn=None,
                                                   scope="z_mean")
        z_stddev = tf.contrib.layers.fully_connected(inputs=new_states, num_outputs=z_size, activation_fn=None,
                                                     scope="z_std")
        ...
        o_stddev = tf.contrib.layers.fully_connected(inputs=new_states, num_outputs=z_size, activation_fn=None,
                                                     scope="o_std")
        samples = tf.random_normal(tf.shape(o_stddev))
        o = o_mean + tf.exp(o_stddev * 0.5) * samples
        vae_y = tf.contrib.layers.fully_connected(inputs=o, num_outputs=label_size, activation_fn=tf.nn.softmax,
                                          scope="vae_classifier")
        loss = tf.reduce_sum(tf.nn.softmax_cross_entropy_with_logits_v2(logits=vae_y, labels=logits))
        latent_loss = 0.5 * tf.reduce_sum(tf.exp(o_stddev) - 1. - o_stddev + tf.square(o_mean), 1)
        encoder_loss = loss + latent_loss
        return states, z_mean, z_stddev,o,encoder_loss


def decoder(decoder_embed_input, decoder_y, target_length, max_target_length, encode_state, batch_size, keep_prob, G,
            reuse=False):
    with tf.variable_scope("decoder", reuse=reuse):
        ...
        decoder_input = tf.nn.embedding_lookup(vae_dic, decoder_input_)
...
def get_cost_l(encoder_embed_input, decoder_embed_input, l_y, decoder_y, target_sequence_length,
               max_target_sequence_length, batch_size, logits, l_a, G, reuse=False):
    encode_states, z_mean, z_stddev,o,encoder_loss = encoder(encoder_embed_input,logits, l_a, keep_prob, reuse)
    ...
    # z_feature = tf.matmul(z, weights_de['w_1']) + biases_de['b_1']  # (64,300)
    # feature_loss = tf.reduce_mean(tf.square(l_a - z_feature))

    z_state = tf.concat([l_y,z,o], 1)  # (64,100+300)
    ...
    decoder_output, predicting_logits, training_logits, masks = decoder(decoder_embed_input, decoder_y,
                                                                        target_sequence_length,
                                                                        max_target_sequence_length,
                                                                        decoder_initial_state, batch_size,
                                                                        keep_prob, G, reuse)

    decoder_input = tf.nn.embedding_lookup(vae_dic, decoder_embed_input)
    ...
    cost = tf.add(encropy_loss, (latentscale_iter * (latent_loss))) + encoder_loss  # cost  (batch_size)
    return cost


# def get_cost_l_all(encoder_embed_input, decoder_embed_input, l_y, decoder_y, target_sequence_length,
#                    max_target_sequence_length, batch_size,logits,l_a,G):
#     flag = 0
...
#                                           target_sequence_length,
#                                           max_target_sequence_length, batch_size,logits,l_a,G,reuse=True)
...
def get_cost_u(u_encoder_embed_input, u_decoder_embed_input):
    logits, logits_class, un_l_a, G = classifier(u_encoder_embed_input, un_target_sequence_length - 1,
                                                 un_max_target_sequence_length - 1, keep_prob=keep_prob, reuse=True)
    class_loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(labels=class_y, logits=logits_class))
    for label in range(label_size):
        y_i = get_onehot(label)
        cost_l = get_cost_l(u_encoder_embed_input, u_decoder_embed_input, [y_i] * un_batch_size, vae_y_u[label],
                            un_target_sequence_length, un_max_target_sequence_length, un_batch_size, logits, un_l_a, G,
                            reuse=True)
    ...
    accuracy = tf.reduce_mean(tf.cast(same_pred, tf.float32))
    # loss_entropy = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(labels=pred_y, logits=logits))

    U = tf.reduce_sum(tf.multiply(L_ulab, prob_y), 1)
    un_label_cost = tf.reduce_mean(U) + class_loss  # + loss_entropy
    return un_label_cost, L_ulab, pred_y, accuracy
...
with tf.name_scope("weight_inital"):
    weights_de = {
        'w_': tf.Variable(tf.random_normal([2*z_size + label_size, vae_hidden], mean=0.0, stddev=0.01)),
...
logits, logits_class, l_a, G = classifier(l_encoder_embed_input, target_sequence_length - 1,
                                          max_target_sequence_length - 1, keep_prob=keep_prob)
...
#                                                             batch_size,logits,l_a,G)
cost_l = get_cost_l(l_encoder_embed_input, l_decoder_embed_input, l_y, vae_y,
                    target_sequence_length, max_target_sequence_length,
                    batch_size, logits, l_a, G)
